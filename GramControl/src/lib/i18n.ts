...
// Cache of locale-code → messages.json contents, populated on first override use.
...
async function loadOverride(locale: LocaleCode): Promise<OverrideMap> {
  ...
  // browser.runtime.getURL is typed against WXT's known public paths; cast
  // because _locales/ is generated outside of the public/ tree.
...
// Kick off initial load. Subscribe to storage changes so manual overrides take
// effect across popup ↔ options without a reload.
...
export function t(key: I18nKey, substitutions?: (string | number)[]): string {
  ...
  // Fall back to browser.i18n (auto-detect path). wxtI18n.t's overload
  // signature is overly strict (fixed-arity tuples per key); the type-safety
  // we care about is enforced at the outer t() signature above.
...
// React hook: returns a render-key that bumps whenever the active locale
// changes, forcing the component subtree to re-translate.
...
// Convenience hook: ensures the override JSON is loaded before first paint
// when the page boots with a non-null userLocale.
